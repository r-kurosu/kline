        ...
        for HOLD in range(1,5):
            for LP in range(1,10):
                for DP in range(1,15):
                    df = df_input[i]
                    df_temp = df[(df['HOLD']==HOLD) & (df['LP']==LP) & (df['DP']==DP)]
                    ...
                    # 出力
                    df_output[i] = df_output[i].append({'ID':count,'HOLD':HOLD,'SEG':seg,'HEIGHT':int(height_ave),'WIDTH':int(width_ave),'AMOUNT':amount,'LP':LP,'DP':DP}, ignore_index = True)
                    count += 1
        ...
        for SEG in range(1,3):
            for LP in range(1,10):
                for DP in range(1,15):
                    ...
                    count += 1
    for i in range(5):            
        df_output[i].to_csv('data/car_group/cargroup'+str(12-i)+'_'+str(booking)+'.csv', index = False)
        df_output_seg[i].to_csv('data/car_group/seggroup'+str(12-i)+'_'+str(booking)+'.csv', index = False)
        # print(df_output[i])
